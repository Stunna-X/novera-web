...
import { jobStatusOptions } from "../../utils/job-utils";
...
const validStatuses = new Set(
  jobStatusOptions.map((option) => option.value),
);
...
export default function JobsPage() {
  ...
  useEffect(() => {
    const status = getStatusFromSearchParams(searchParams);
  ...
  if (!activeOrganizationId) {
    return <WorkspaceEmptyState returnTo="/jobs" />;
  }

  if (!canRead) {
    return (
      <AccessDenied description="Your current workspace role does not include work_orders.read." />
    );
  }
      ...
      <section className="flex flex-col gap-5 rounded-3xl border border-slate-800 bg-slate-900/55 p-6 sm:flex-row sm:items-end sm:justify-between sm:p-8">
        <div>
          ...
          <p className="mt-3 max-w-2xl text-sm leading-7 text-slate-500">
            Plan, assign, monitor, and close field work for{" "}
