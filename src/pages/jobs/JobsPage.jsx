...
export default function JobsPage() {
  ...
  const pageSize = 20;
  const canRead = hasPermission("work_orders.read");
  const canCreate = hasPermission("work_orders.create");

  useEffect(() => setPage(1), [deferredSearch, filters.includeInactive, filters.priority, filters.status]);

  const { items, total, loading, error, reload } = useJobs({
  ...
  if (!activeOrganizationId) return <WorkspaceEmptyState returnTo="/jobs" />;
  if (!canRead) {
    return <AccessDenied description="Your current workspace role does not include work_orders.read." />;
  }

  const pageCount = Math.max(1, Math.ceil(total / pageSize));
  const filtered = Boolean(filters.search || filters.status || filters.priority || filters.includeInactive);
  ...
  return (
    <div className="space-y-6">
      <section className="flex flex-col gap-5 rounded-3xl border border-slate-800 bg-slate-900/55 p-6 sm:flex-row sm:items-end sm:justify-between sm:p-8">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-emerald-300">Operations</p>
          <h1 className="mt-3 text-2xl font-semibold tracking-tight text-white sm:text-3xl">Jobs</h1>
          <p className="mt-3 max-w-2xl text-sm leading-7 text-slate-500">Plan, assign, monitor, and close field work for {activeOrganization?.name}.</p>
        </div>
      ...
      <JobFilters filters={filters} onChange={setFilters} onReset={() => setFilters(defaultFilters)} />

      {error && <Alert variant="error">{getApiErrorMessage(error, "Unable to load jobs.")}</Alert>}

      {loading && items.length === 0 ? (
        <div className="h-80 animate-pulse rounded-2xl bg-slate-900/60" />
      ) : items.length === 0 ? (
        <JobsEmptyState canCreate={canCreate} filtered={filtered} />
      ) : (
      ...
      {total > 0 && (
        <div className="flex flex-col gap-3 rounded-2xl border border-slate-800 bg-slate-900/55 px-4 py-3 text-xs text-slate-500 sm:flex-row sm:items-center sm:justify-between">
          <p>Showing {(page - 1) * pageSize + 1}–{Math.min(page * pageSize, total)} of {total} jobs</p>
